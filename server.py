...
# Ensure naturalizer is importable even without pip install or PYTHONPATH.
# This lets `python server.py` work when CWD is the repo root.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _route_post(self) -> None:
        ...
        if path == "/api/export":
            ...
            chain_mode = str(data.get("chain_mode", "standard")).strip().lower()
            if chain_mode not in {"standard", "extended", "multi", "hybrid", "best"}:
                chain_mode = "standard"
            if not features["llm"]:
                if use_llm:
                    plan_note = (
    ...
    def _handle_upload(self, parsed) -> None:
        ...
        plan_note = None
        use_llm = False
        chain_mode = str(data.get("chain_mode", "standard")).strip().lower() if data else "standard"
        if chain_mode not in {"standard", "extended", "multi", "hybrid", "best"}:
            chain_mode = "standard"
    ...
    def _status() -> dict:
        ...
        try:
            from naturalizer.llm import llm_provider_choices
        ...
        return {
            ...
            "llm_configured": _llm_on(),
            "llm_model": llm_provider_label() if _llm_on() else None,
